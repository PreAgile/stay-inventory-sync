...
| 채널과 숫자가 어긋남 | 키 단위 버전 스탬프 | diff 리포트 · 재동기화(**앞 500건만**) | **부분** |
| 인바운드 **중복** | Inbox 3중 멱등 | 워커 재시도 | 완결 |
| 인바운드 **순서** | 순서키 정렬 | — | **부분** |
| DB 장애 | `lock_timeout` · 풀 상한 | — | **부분** |
| 인스턴스 증가 | 릴레이만 집기-임대 | — | **부분** |
...
- **재동기화는 격자의 앞 500건만 반복한다** — 커서가 없다. 최종 방어선이
  부분만 덮는다 (`#71`)
...
| 채널과 숫자가 다르다 | `/ops/inventory-diff` | 원인 판정 → 재동기화 대기 또는 캡 조정 |
