...
**테스트 264개** 중 **241개가 Testcontainers 위의 실제 PostgreSQL** 에서 돈다.
...
| 제외 | 이유 |
|---|---|
| 게스트·사장님용 프론트엔드 | 백엔드 문제이며, 검증 대상은 API 계약과 정합성 |
| 그 밖의 화면 | **운영 콘솔 한 장은 만들었다** (`#80`). §7 의 제목이 *"무엇을 보고 무엇을 누르나"* 인데 그 답이 `curl` 이면 주장과 산출물이 어긋난다. 빌드 단계도 프레임워크도 없는 HTML 한 파일이고 **거기서 멈추는 것이 범위 판단이다** |
...
**이 표의 왼쪽과 오른쪽이 한 화면에 있다** — `/ops/console` (`#80`). 위 다섯 줄 중 넷은
거기서 보고 거기서 누른다. 운영자에게 `curl` 을 요구하면 확인 업무를 없앤 것이 아니라
**개발자에게 떠넘긴 것**이다.
...
**`OPS_API_KEY` 를 빼면 애플리케이션이 뜨지 않는다.** 그렇게 만든 것이다 —
기본값을 두면 그 값이 곧 공개 키이고, `${OPS_API_KEY:}` 처럼 빈 기본값을 두면 Spring 이
그것을 **리터럴 문자열로 넘겨** 누구나 그 문자열을 헤더로 보내 들어온다.
설정된 것처럼 보이는 미설정이 미설정보다 나쁘다. 그래서 없으면 뜨지 않게 했고,
그때 나오는 것은 이 줄이다.
...
띄운 뒤 <http://localhost:8080/ops/console> 을 열고 위에서 준 키를 넣는다.
지표 · DEAD 재투입 · 재고 diff · 재동기화가 한 화면에 있다 (`#80`).

**이 주소만 키 없이 열린다.** 브라우저 주소창으로 여는 최상위 이동에는 헤더를 붙일
수단이 없어서인데, **열리는 것은 데이터가 아니라 빈 문서다** — 숫자는 전부 브라우저가
`X-Ops-Key` 를 붙여 따로 가져온다. `OpsSecurityTest` 가 둘을 각각 검사한다.
키 없이 열리는 `/ops` 경로가 **이 하나뿐인 것**과, **그 하나에 데이터가 없는 것**.

### 주장마다 어떤 명령으로 확인하나

이 문서가 하는 주장을 하나씩 직접 확인하는 명령은
[`docs/09-how-to-verify.md`](docs/09-how-to-verify.md) 에 있다.
**README 를 다 읽지 않고 확인부터 하려면 거기서 시작한다.**
