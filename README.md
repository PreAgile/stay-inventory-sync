...
설계가 아니라 **구현하면서 드러난 것**만 적는다. 여섯 건 모두 테스트가 먼저 알려 줬다.
...
**해결** — 관측 가능한 결과가 아니라 **판정 쿼리 자체**를 보는 테스트를 추가했다.
...
그 규칙이 성립할 수 없고, 성립하지 않는 규칙을 적어 두면 검사가 아니라 장식이 된다.
