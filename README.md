...
> **숙박 PMS 전체를 만들지 않았다.** 다중 채널 예약에서 가장 비싼 장애인
> **재고 불일치를 막고 · 발견하고 · 되돌리는 최소 코어**만 구현했다.
> 무엇을 잘라냈고 왜 잘랐는지는 [3. 범위](#3-범위)에 적었다.
>
> 스스로 반증해서 찾아낸 정합성 결함 세 건(`#64` · `#71` · `#72`)은 경위까지 남긴 뒤 닫았고,
> **아직 비어 있는 공백 여덟 개**는 [`08-failure-and-recovery.md`](docs/08-failure-and-recovery.md) §9 에
> 착수 조건과 함께 적었다. 완성된 PMS 라고 주장하지 않는다.
...
두 경우 모두 결과는 **오버부킹**이다.
비용은 현장이 지불한다. 환불, 룸 이동, CS 대응, 그리고 그 모든 것을 조율하는 사람의 시간.
...
이 중 **가장 많은 것을 드러내는 쪽은 확인 업무다.**

수기 업무는 도구가 아직 없어서 생긴다. 만들면 해결된다.
...
| A2 | 재고는 **날짜별 잔여 수량**으로 관리된다 | 다일 예약이 여러 행에 걸친다 |
...
날짜에는 노출 상한을 미리 설정할 수 없다** (캡형 운영에서 실제로 필요한 동작이다).
...
**③ 세 개의 `UNIQUE` 가 멱등성을 DB 에 맡긴다.** 제약이 판정한다.
...
`sequence_key` 는 NULL 일 수 있고(순서키를 주지 않는 채널이 있다) PostgreSQL 은
기본적으로 **NULL 을 NULL 과 같다고 보지 않는다.** 뚫리는 곳은 순서키를 주지 않는
채널뿐이라 조용하다. `NULLS NOT DISTINCT` 는 PostgreSQL 15 이상이며 이것이 스택 하한을
고정한다.
...
이 테이블들이 지켜야 하는 것을 적는다. 테스트가 실제로 검사하는 대상이다.
...
부수 효과로 전이표를 **DB 없이** 단위 테스트로 고정할 수 있다.
...
| 실패 | 방어 | 회복 | |
|---|---|---|---|
| 예약 요청이 애매하게 끝남 | 멱등 키를 호출부가 준다 · 2단 방어 | 재시도가 같은 답 | 완결 |
...
| DB 장애 | `lock_timeout` · 풀 상한 · **`503` 매핑** | 호출부가 재시도 시점을 안다 | 완결 |
...
### 가장 컸던 구멍 — 서버가 멱등 키를 만들고 있었다

`channelReservationId` 를 안 주면 서버가 요청마다 UUID 를 만들었고,
**타임아웃 후 재시도가 중복 예약이 됐다.** `UNIQUE` 는 살아 있는데 판정 대상이
사라진 경우다 — **멱등은 서버가 혼자 만들 수 없다.**

`#64` 로 닫았다(ADR-0014). 키는 **호출부가 준다** — 채널 예약은
`channelReservationId`, 직접 예약은 `Idempotency-Key` 헤더, 둘 다 없으면 `400` 이다.
중복은 `200` 으로 **기존 예약을 돌려준다.**
...
실제로는 애초에 걸린 적이 없다. **사람이 부르는 API 에서는 멱등이 오히려
거짓말이 된다.**

**임계값을 정했다** (`#68`). 근거는 **채널 레이트 리밋과 예약 리드타임**이다.
**같은 지연이라도 날짜에 따라 허용치가 다르다.** 오늘 밤 재고의 통보
지연 5분과 6개월 뒤 재고의 5분은 다른 사건이다. 표는
[`08-failure-and-recovery.md`](docs/08-failure-and-recovery.md) §8 에 있다.

**헬스체크는 liveness 와 readiness 를 나눈다** (`#69`). `readiness` 만 DB 를 본다.
`liveness` 가 DB 를 보면 **DB 장애로 오케스트레이터가 살아 있는 프로세스를 죽여
회복을 방해한다.**

**`/ops` 는 정적 API 키로 막는다** (`#74`). 패턴으로 막으므로 엔드포인트가 늘 때
자동으로 적용되고, 키가 비어 있으면 **애플리케이션이 뜨지 않는다.** "설정을 잊으면
인증이 꺼진다" 는 형태를 두지 않는다.
...
**원인** — 복원이 한 번만 일어난 게 아니었다. 초과 복원이 `CHECK (sold >= 0)` 에
걸려 예외로 죽었고 그 예외가 `pool.submit` 의 `Future` 안에 갇혀 **아무 데도 나타나지
않았다.** 테스트는 초록불인데 절반이 죽어 있었고 그 상태로 나온 숫자를 근거로
*"정확히 5건"* 을 주장하고 있었다.

**해결** — 동시성 헬퍼를 공용으로 빼고 **새어 나온 예외를 모아 실패로 드러낸다.**
...
**해결** — 소진 판정은 *"몇 번 실패했는가"* 를 센다. *"몇 번 미뤘는가"* 는 세지 않는다.
...
ADR 에는 **축이 늘 때마다 그 축의 직렬화 지점을 찾아야 한다**고 적었다.
...
Kotlin 은 의도적으로 **좁은 부분집합만** 사용한다.
`data class`, `sealed interface`, null 안전성, `val` 기본까지.
코루틴, DSL, `inline reified`, context receivers 는 쓰지 않는다.
...
JPA 엔티티에는 `data class` 를 쓰지 않는다. 이유도 ADR-0005 에 있다.

JPA 는 쓰되 **엔티티 간 연관 매핑을 두지 않는다.** dirty checking 이 아키텍처 규칙을 우회해
재고를 바꾸는 경로를 만들기 때문이다. 채택 근거와 기각한 대안(jOOQ, R2DBC 등)은
[ADR-0008](docs/adr/0008-jpa-with-explicit-writes.md).

H2 는 사용하지 않는다. `SELECT FOR UPDATE` 의 동작이 PostgreSQL 과 달라
락 검증이 성립하지 않기 때문이다.
...
테스트가 전부 통과하면 이 프로젝트가 주장하는 것이 증명된다.
...
이 문서들을 근거로 삼았다. **200개 이상의 PMS가 라이브 전에
...
조사한 오픈소스 PMS는 공통적으로 기능의 폭으로 승부한다.
채널 동기화는 cron 폴링으로 처리한다. 정합성 검증은 명시적으로 수행하지 않는다.
