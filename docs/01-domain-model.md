...
| | 이유 |
|---|---|
| ① | **참조 대상이 하나가 아니다.** `kind` 가 `BOOKING` 이면 예약을, `POLICY` 면 `(룸타입, 날짜, 채널)` 을 가리킨다. 컬럼 하나로 두 축을 표현할 수 없다 |
...
>
> 안 두는 이유는 위 셋이다. 특히 ①이 있는 한 nullable FK 로도 **예약 축만** 표현된다.
> `POLICY` 알림은 그 컬럼이 영구히 NULL 이고, 그러면 "NULL 인 이유"가 두 가지로 섞인다 —
> 아직 안 풀렸는가, 애초에 예약이 아닌가. 상태를 그 컬럼으로 읽을 수 없게 된다.
