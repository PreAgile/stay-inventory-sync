...
**이 선택의 대가는 중복 저장만이 아니다.** FK 에 `ON UPDATE` 절이 없으므로 기본값
`NO ACTION` 이 적용된다 — **활성 선점이 남아 있는 동안 `reservation.room_count` 나
`room_type_id` 를 변경하면 거부된다.**

이것을 제약이 아니라 **의도로 읽어야 한다.** 선점은 "이 수량만큼 이 룸타입을 잡아
두었다" 는 약속이고, 그 밑에서 수량을 바꾸면 잡아 둔 것과 팔려는 것이 달라진다.
수량·룸타입 변경은 **선점을 해제한 뒤에** 한다. 예약 변경 API 가 생기면 그 순서를
따라야 한다.
...
없으면 **재시도된 선점 요청이나 동시 요청 두 건이 행을 둘 만든다.** 그러면 `INV-4` 의
유효 선점 합계가 `room_count` 를 두 번 세고, **3객실 예약이 6으로 읽혀** 남은 재고를
실제보다 적게 판정한다. 오버부킹이 아니라 **조용한 기회손실**이다.

부분 인덱스인 이유는 두 가지다. `released_at` 이 채워진 과거 선점은 이력으로 남아야
하고, **해제 후 같은 날짜를 다시 선점하는 것은 정상 경로**다.
