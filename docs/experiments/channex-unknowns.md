# 실측 계획 — 연동 대상이 정해진 뒤에 실행한다

**지금 실행할 문서가 아니다.**

`docs/07-reconciliation.md` 의 설계는 특정 채널매니저에 의존하지 않는다.
이 문서는 **실제로 연동을 붙이는 시점**에 상대가 무엇을 주는지 확인하는 절차이며,
Channex 를 예로 삼는다 — 공개 문서와 자체 가입 가능한 staging 이 있어 측정이 가능하기 때문이다.

세 항목은 **보장형 하이브리드(ADR-0001)의 성립 여부**를 가른다.
연동 대상이 정해지지 않은 상태에서는 보장형을 착수하지 않으므로, 측정도 미룬다.
